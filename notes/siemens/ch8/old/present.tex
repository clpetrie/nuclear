...
\begin{itemize}
   ...
   \begin{equation}
      ...
      \label{equ:8.4.16}
   \end{equation}
   \item This is called the characteristic equation of the RPS (Random Phase Approximation).
\end{itemize}
}
...
\framesubtitle{RPA Response to an External Field}
\begin{itemize}
   \item This RPS response tensor describes the total force on the external field due to the nucleons' rearrangement.
...
\begin{itemize}
   \item To do this we need to be able to determine the collective degrees of freedom $Q$, and the associated form factors,
   \begin{equation}
      F_\mu^{sp} = \frac{\partial H^{sp}}{\partial Q_\mu} \bigg\rvert_{Q_\alpha = Q_\alpha^0}
   \end{equation}
   \item Let's assume that we know the ground state states, energies, and Hamiltonian, $H^{sp}_0\ket{k} = \hat{\varepsilon}_k\ket{k}$.
   \item Now let's use the definition to calculate
   \begin{equation}
...
\framesubtitle{Vibrational Parameters: the Coupling Constant and the Sum Rule}
\begin{itemize}
   \item Now, like Bohr and Mottelson, let's use the potential in 8.3.1, and look at vibrations about a spherical equilibrium to obtain an estimate of the coupling constant in 8.4.20.
   \begin{align}
...
\framesubtitle{Vibrational Parameters: the Coupling Constant and the Sum Rule}
\begin{itemize}
   \item There is another way to calculate $\kappa_{\mu\nu}$ that only requires first order variables.
...
\begin{itemize}
   ...
   \end{equation}
   which is the same as equation 8.4.37.
\end{itemize}
}
...
\begin{itemize}
   ...
   \begin{equation}\begin{split}
      ...
      &- \frac{1}{2}\sum\limits_{\mu\nu}\kappa_{\mu\nu}(Q_\mu-Q_\mu^0)(Q_\nu-Q_\nu^0)
   \end{split}\end{equation}
   we see that $S_{\mu\nu}$ is the same wether we use $H_{sp}(\{Q_\alpha\})$ or $H_{sp}(\{Q_\alpha^0\})$, and thus we can write
   \begin{align}
