...
We have come up with two such correlations, the exponentially,
\begin{equation}
...
\begin{equation}
   \Psi_{\text{SP}}(\R, S) = \left[\prod\limits_{i<j}f_c(r_{ij})\right] \left[\mathcal{S}\prod\limits_{i<j}\left(1+\sum\limits_p\fpij\Opij\right)\right] \ket{\phi}.
   \label{equ:prodpsi}
\end{equation}
...
The exponentially correlated wave function explicitely obeys cluster decomposition given that the correlating functions, $f_p(r_{ij})$ go to zero as the particle seperation increases. \red{Start here, and give a better description of this last sentence.}

The symmetrized product wave function, introduced by Pandharipande and Wiringa in 1979 \cite{pandharipande1979}, \ldots
\red{\\ Explain the similar properties like the variables and then dive into each of them to explain how they fit the properties that we want.}
