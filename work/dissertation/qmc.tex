...
It is difficult to operate through the Green's function and maintain low variance and so observables are often computed via mixed expectation values
\begin{equation}
...
\begin{figure}[h!]
   ...
   \caption[Sample Data Showing the Convergence of the Statistical Uncertainty as the Block Size Is Increased. An Appropriate Block Size for This Data Would Be about 35, Where the Errors Stop Increasing with Block Size.]{Sample data showing the convergence of the statistical uncertainty as the block size is increased. An appropriate block size for this data would be about 35, where the errors stop increasing with block size.}
   \label{fig:blockaverage}
\end{figure}

Good reviews of this method can be found in \cite{foulkes2001} and  \cite{carlson2015}. DMC only accounts for spin-isospin independent Hamiltonians, unlike Green's function Monte Carlo (GFMC) and auxiliary field diffusion Monte Carlo (AFDMC) which follow DMC for sampling spatial states but use two different methods to sample spin-isospin states. For each set of spatial integrals there is a sum over all of the spin and isospin states. GFMC evaluates this sum explicitly. This is inefficient as the number of spin-isospin charge states scales as
\begin{equation}
   \frac{A!}{Z!(A-Z)!}2^A,
\end{equation}
...
In this work the AV6$'$ potential is used, which includes all of the same operators as AV8$'$ except for those including the spin-orbit terms, $\left[1,\si\cdot\sj,S_{ij}\right]\otimes\left[1,\ti\cdot\tj\right]$. The functions multiplying the spin-isospin dependent AV6$'$ operators are shown in Figure \ref{fig:vij}.
\begin{figure}[h!]
   \centering
   \includegraphics[width=0.7\textwidth]{figures/vij.eps}
   \caption[The Functions Multiplying the AV6$'$ Operators as a Function of Nucleon Separation. The Central Operator, $v_1$, Is Excluded to Show the Spin-isospin Terms with Better Detail. The $v_2$ Represents the Isospin Operators, $v_3$ the Spin Operators, $v_4$ the Spin-isospin Operators, $v_5$ the Tensor, and $v_6$ the Tensor-isospin Operators.]{The functions multiplying the AV6$'$ operators as a function of nucleon separation. The central operator, $v_1$, is excluded to show the spin-isospin terms with better detail. The $v_2$ represents the isospin operators, $v_3$ the spin operators, $v_4$ the spin-isospin operators, $v_5$ the tensor, and $v_6$ the tensor-isospin operators.}
   \label{fig:vij}
\end{figure}
...
\begin{figure}[h!]
   ...
   \caption[Term by Term Expansion of the Chiral Perturbation Theory Grouped by Powers of $(Q/\lambda_\chi)^\nu$. Solid Lines Represent Nucleons and Dashed Lines Represent Pions. Figure Used with Permission from \cite{entem2017}.]{Term by term expansion of the chiral perturbation theory grouped by powers of $(Q/\Lambda_\chi)^\nu$. Solid lines represent nucleons and dashed lines represent pions. Figure used with permission from \cite{entem2017}.}
\end{figure}
...
\begin{figure}[h!]
   ...
   \caption[Scaling on Supermic Using the Time to Propagate 10,000 Configurations of an $^{16}$O Nucleus for 100 Steps.]{Scaling on SuperMIC using the time to propagate 10,000 configurations of an $^{16}$O nucleus for 100 steps.}
   \label{fig:supermic}
\end{figure}
\begin{figure}[h!]
   \centering
   \includegraphics[width=0.8\textwidth]{figures/stampede.eps}
   \caption[Scaling on Stampede2 Using the Time to Propagate 10,000 Configurations of an $^{16}$O Nucleus for 100 Steps.]{Scaling on Stampede2 using the time to propagate 10,000 configurations of an $^{16}$O nucleus for 100 steps.}
   \label{fig:stampede2}
\end{figure}
