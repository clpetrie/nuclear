...
\isotope[16]{O}. \green{Softer interactions manifest instead a significant dependence 
...
more efficiently up \isotope[12]{C}, and to go beyond $A=12$.
...
\begin{table}[tb]
\centering
\caption[]{\isotope[4]{He} ground-state energies for the AV6$'$ potential and different 
...
\caption[]{Energy of \isotope[4]{He} as a function of 
...
and \isotope[4]{He}, for which the constrained energy is below the GFMC prediction, considered to be the 
...
for \isotope[4]{He} with the same interaction (with AV6$'$ of~\cref{fig:tr_he4}).
...
In \isotope[6]{He} the difference between the constrained and unconstrained energy is of 
the order of $1\,\rm MeV$, roughly $3\%$ of the final result. 
In \isotope[16]{O} instead, the constrained energy is higher, 
...
Though there is little difference in \isotope[4]{He}, the constrained energies 
for both \isotope[16]{O} and SNM are lower when employing quadratic correlations, 
particularly for SNM. In \isotope[16]{O} the energy gain for the constrained
...
observables: the binding energy of \isotope[4]{He} and $n$-$\alpha$
scattering $P$-wave phase shifts.
This choice probes properties of light nuclei (the \isotope[4]{He}
...
\isotope[16]{O} was found for this softer cutoff and the $E\tau$
...
LO and N$^2$LO $E\tau$ results for \isotope[16]{O} with $R_0=1.2\,\rm fm$ are outside the displayed energy region.
...
For the heaviest system considered here, \isotope[16]{O}, the picture is quite different. 
...
\Cref{fig:rch} shows the charge radii at different orders of the chiral 
...
good all the way up to oxygen. One exception is \isotope[6]{Li}, for which the charge radius
...
with experimental data, except for \isotope[6]{Li} (also shown in~\cref{tab:afdmc-gfmc}). 
The picture changes again for $A=16$. The charge radius of \isotope[16]{O} turns out to 
be close to $2.2\,\rm fm$ with the $E\tau$ parametrization of the $3N$ force, 
smaller than that of \isotope[6]{Li} for the same potential, but consistent with the 
...
The full calculation of \isotope[12]{C} at N$^2$LO required on the
...
As shown in~\cref{tab:10,tab:12}, the overbinding in \isotope[16]{O} happens only when the $3N$ force 
...
\isotope[6]{Li} and \isotope[16]{O} (\cref{tab:v3}) clearly shows the issue. 
A large negative $V_D$ contribution in \isotope[16]{O} for the soft $E\tau$ potential drives the system to 
...
$\langle V_E\rangle$. This is still true in \isotope[6]{Li} also for $R_0=1.2\,\rm fm$, but 
the balance is broken for the soft $E\tau$ potential in \isotope[16]{O}. The main reason for such
...
%To confirm this hypothesis, we tried to perturbatevly calculate $\langle V_D\rangle$ 
%in the $E\tau$ parametrization, i.e. we removed the $V_D$ contribution from the imaginary-time propagation, 
%but we calculated its expectation value. For the soft potential we obtained 
%$\langle V_D\rangle_{\rm pert}^{1.2\,\rm fm}=-78(5)\,\rm MeV$, a large negative contribution in line with 
%the real expectation value of~\cref{tab:v3}.
%For the hard potential in the $E\tau$ parametrization $c_D=0$, so we artificially set $c_D=1.0$.
%The result is $\langle V_D\rangle_{\rm pert}^{1.0\,\rm fm}=-23(2)\,\rm MeV$, $\approx 3.4$ smaller
%than $\langle V_D\rangle_{\rm pert}^{1.2\,\rm fm}$.
...
The charge form factors of \isotope[6]{Li}, \isotope[12]{C}, and \isotope[16]{O} are shown 
...
%Comment: for the hard interaction, the corresponding cutoff in momentum space is 
...
The charge form factor of \isotope[6]{Li} for the $E\mathbbm1$ interaction is compatible
...
A similar physical picture is given for both \isotope[12]{C} and \isotope[16]{O}, 
...
\isotope[16]{O} is less accurate, with the position of the first diffraction 
...
of \isotope[4]{He} and \isotope[6]{Li} very close to those shown in~\cref{fig:sl}. 
An exception is the case of \isotope[16]{O}, for which $S_L(q)$
...
Binding energies and charge radii are shown for all the systems,
and results for the charge form factor in \isotope[6]{Li}, \isotope[12]{C}, and 
\isotope[16]{O} are also reported. For all these observables, the AFDMC results
are supported by statistical Monte Carlo errors and theoretical errors coming
from the truncation of the chiral expansion. Finally, the Coulomb sum rule for 
systems with $4\leq A\leq 16$ is also shown.

The outcomes of this work confirm that harder local chiral interactions fit to 
few-body observables give a very good description of the ground-state properties 
of nuclei up to \isotope[16]{O}. 
\green{Softer interactions manifest instead a significant dependence 
on the employed operator structure and possibly call for the necessity of 
inputs other than few-body observables.}


\acknowledgments{We thank I.~Tews, A.~Lovato, A.~Roggero, and R.~F.~Garcia Ruiz 
...
at LANL.
K.E.S. and C.P. were supported by the National Science Foundation grant PHY-1404405.
C.P. was also supported in part by a Wally Stoelzel Fellowship from the
Department of Physics at Arizona State University.
The work of J.E.L. and A.S. was supported by the ERC Grant No.~307986
...
contract DE-AC02-05CH11231, by the Lichtenberg high performance
computer of the TU Darmstadt, and by the Extreme Science and Engineering
