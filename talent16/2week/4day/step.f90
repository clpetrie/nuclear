module step
   ...
   real(kind=r8), private, parameter :: delta=0.1
   real(kind=r8), private, parameter :: delta2=1.0
   real(kind=r8), private, parameter :: delta3=1.0
...
!   end subroutine
   ...
   subroutine metropolis(phi,accepted)
   ...
   real(kind=r8) :: act
   integer(kind=i4) :: ts !t slice that get changed
   ...
   do nx=0,L-1
      do ny=0,L-1
         do nz=0,L-1
            phinew(nx,ny,nz,ts)=phi(nx,ny,nz,ts)+delta*(ran(irn)-0.5_r8)
         enddo
      enddo
   enddo
!   ratio=exp(-SE(phinew))/exp(-SE(phi))
   ratio=exp(-SE(phinew)+SE(phi))
   ...
   subroutine metropolis2(phi,accepted)
   real(kind=r8), dimension(0:L-1,0:L-1,0:L-1,0:Lt-1) :: phi
   real(kind=r8), dimension(0:L-1,0:L-1,0:L-1,0:Lt-1) :: phinew
   real(kind=r8) :: ratio,acceptance,rand1
   integer(kind=i4) :: nx,ny,nz,nt
   real(kind=r8) :: act
   integer(kind=i4) :: xs,ys,zs,ts !t slice that get changed
   real(kind=r8) :: accepted
   ts=nint(ran(irn)*10.0_r8-0.5_r8) !integer from 0 to 9
   ...
   phinew=phi
   phinew(xs,ys,zs,ts)=phi(xs,ys,zs,ts)+delta2*(ran(irn)-0.5_r8)
!   ratio=exp(-SE(phinew))/exp(-SE(phi))
   ratio=exp(-SE(phinew)+SE(phi))
   if (ran(irn).lt.ratio) then
      phi=phinew
   ...
   subroutine metropolis3(phi,accepted)
   real(kind=r8), dimension(0:L-1,0:L-1,0:L-1,0:Lt-1) :: phi
   real(kind=r8), dimension(0:L-1,0:L-1,0:L-1,0:Lt-1) :: phinew
   real(kind=r8) :: ratio,acceptance,rand1
   integer(kind=i4) :: nx,ny,nz,nt
   ...
   do nx=0,L-1
      do ny=0,L-1
         do nz=0,L-1
            do nt=0,L-1
               phinew(nx,ny,nz,nt)=phi(nx,ny,nz,nt)+delta3*(ran(irn)-0.5_r8)
               ratio=exp(-SE(phinew)+SE(phi))
               if (ran(irn).lt.ratio) then
                  phi=phinew
                  accepted=accepted+1.0_r8/(L**3.0_r8*Lt)
               endif
            enddo
         enddo
      enddo
